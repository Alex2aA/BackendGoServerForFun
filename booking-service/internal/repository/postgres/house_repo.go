...
func (r *HouseRepository) Create(ctx context.Context, house *domain.House) error {
	_, err := r.db.Exec(ctx, `
		INSERT INTO free_houses (id, id_user, id_hostel, address, number_of_rooms, price_for_one_day, name, description)
...
func (r *HouseRepository) GetByID(ctx context.Context, id string) (*domain.House, error) {
	...
	err := r.db.QueryRow(ctx, `
		SELECT id, id_user, id_hostel, address, number_of_rooms, price_for_one_day, name, description 
		FROM free_houses WHERE id = $1`, id).
		Scan(&h.ID, &userID, &h.HostelID, &h.Address, &h.NumberOfRooms, &h.PricePerDay, &h.Name, &h.Description)

	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, nil
		}
		logger.Log.Error("GetByID failed", zap.String("house_id", id), zap.Error(err))
		return nil, err
	}
	if userID != nil {
		h.UserID = *userID
